...
namespace SistemaVenta.BLL.Servicios
{
    public class MenuService : IMenuService
    {
        ...
        {
            IQueryable<Usuario> tbUsuario = await this._usuarioRepositorio.Consultar(u => u.IdUsuario == idUsuario);
            IQueryable<MenuRol> tbMenuRol = await this._menuRolRepositorio.Consultar();
            IQueryable<Menu> tbMenu = await this._menuRepositorio.Consultar();

            try
            {
                IQueryable<Menu> tbResultado = (from u in tbUsuario
                                                join mr in tbMenuRol
                ...
                return this._mapper.Map<List<MenuDTO>>(listaMenus);
            }
            catch
            {
                throw;
